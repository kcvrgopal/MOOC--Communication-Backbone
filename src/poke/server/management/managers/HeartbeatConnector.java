...
public class HeartbeatConnector extends Thread {
	...
	public void addConnectToThisNode(HeartbeatData node) {
		...
		HeartMonitor hm = new HeartMonitor(node.getNodeId(), node.getHost(),
				node.getMgmtport());
	...
	public void run() {
		if (monitors.size() == 0) {
			...
			logger.info("HB connection monitor starting, node has "
					+ monitors.size() + " connections");
	...
	private void validateConnection() {
		...
		for (HeartbeatData hb : HeartbeatManager.getInstance().incomingHB
				.values()) {
			// receive HB - need to check if the channel is readable
			if (hb.channel == null) {
				if (hb.getStatus() == BeatStatus.Active
						|| hb.getStatus() == BeatStatus.Weak) {
				...
				if (hb.channel.isWritable()) {
					if (System.currentTimeMillis() - hb.getLastBeat() >= hb
							.getBeatInterval()) {
						hb.incrementFailures();
		...
		// validate connections this node wants to create
		for (HeartbeatData hb : HeartbeatManager.getInstance().outgoingHB
				.values()) {
