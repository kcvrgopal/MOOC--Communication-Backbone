...
public class Server {
	...
	private static class StartCommunication implements Runnable {
		...
		public void run() {
			...
			try {
				...
				//Listening to External Common port(7000) and Server port
				ServerBootstrap b = new ServerBootstrap();
				bootstrap.put(port, b);
				bootstrap.put(7000, b);
				...
				ChannelFuture f = b.bind(7000).syncUninterruptibly();
				ChannelFuture f1 = b.bind(port).syncUninterruptibly();
	...
	private void startManagers() {
		...
		//Creating the Election Manager
		electionMgr = ElectionManager.getInstance(myId, votes);
		Comm.LeaderElection.Builder builder = LeaderElection
				.getDefaultInstance().newBuilder();
		builder.setNodeId(myId);
		builder.setVote(Comm.LeaderElection.VoteAction.ELECTION);
		builder.setDesc("Leader Election");
		builder.setBallotId(myId);
		LeaderElection leaderElection = builder.build();
		//Entry point of Leader Election
		electionMgr.startElection(leaderElection, conf);
	}
