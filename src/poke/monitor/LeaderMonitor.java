...
public class LeaderMonitor {
	...
	public LeaderMonitor(String host, String port) {
		this.host = host;
		this.port = Integer.parseInt(port);
		this.group = new NioEventLoopGroup();
	}
	public LeaderMonitor(String host, int port) {
		this.host = host;
		this.port = port;
		this.group = new NioEventLoopGroup();
	}
	...
	//	while(true){
		if (channel == null) {
			try {
				...
				logger.info(host+";"+port+"IN Leader monitor");
	...
	public void startVoting(Request request) {
		...
		try {
			Channel ch = connect();			
			ch.writeAndFlush(request);
			System.out.println("CHECK ME"+request);
			rtn = true;
			} catch (Exception e) {
				logger.error("could not send connect to node", e);
		}
