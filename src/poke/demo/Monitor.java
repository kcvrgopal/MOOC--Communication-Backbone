	...
	public static class HeartPrintListener implements MonitorListener {
		...
		public void onMessage(Management msg) {
			...
			if (!nodeID.equals(msg.getBeat().getNodeId()))
				...
				logger.info("Received graph responses from "
						+ msg.getBeat().getNodeId());
			} else if (msg.hasBeat()) {
				logger.info("Received HB response: "
						+ msg.getBeat().getNodeId()+" counter is "+HeartbeatManager.count);
			} else
				logger.error("Received management response from unexpected host: "
						+ msg.getBeat().getNodeId());
		}
